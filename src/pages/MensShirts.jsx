...
import { useGetMensShirtsQuery } from "../features/apiSlice";
...
function MensShirts() {
  ...
  const {
    ...
    isError,
    isLoading,
  } = useGetMensShirtsQuery({ limit: 100, skip: 0 });

  if (isLoading) return <h1>Loading....</h1>;
  if (isError) return <div>Error: {error.toString()}</div>;

  const loopmensShirts = repeatProducts(mensShirts.products);
  console.log(mensShirts);

  return (
    <div className="p-4">
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {loopmensShirts.map((product) => (
